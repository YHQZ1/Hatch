...
export default function NewProject() {
  ...
  return (
    // REMOVED h-[calc(100vh-80px)] and mt-20. Layout handles the top spacing.
    <div className="flex flex-col lg:flex-row h-[calc(100vh-80px)] relative z-10">
      <section className="w-full lg:w-[460px] border-r border-[var(--border)] bg-[var(--bg)] flex flex-col z-20">
          ...
          {step === 1 && (
            <div className="p-8 space-y-6">
              ...
              <input
                ...
                className="w-full bg-[var(--surface)] border border-[var(--border)] px-4 h-11 text-sm font-mono outline-none focus:border-[var(--border-focus)] transition-all placeholder-[#333] text-white"
              />
          ...
          {step === 2 && selectedRepo && (
            <div className="p-8 space-y-8">
              ...
              {!checkingDocker && hasDockerfile === false && (
                <div className="border border-red-900/50 bg-red-900/10 px-4 py-3 flex items-start gap-3">
                  <span className="text-red-500 text-xs mt-0.5">✕</span>
                  <p className="font-mono text-[9px] text-red-500/80 leading-relaxed uppercase tracking-wider">
                    No Dockerfile detected at root. Deployment will fail.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-6">
                <Field label="Branch" sub="Source branch to deploy from">
                  <input
                    ...
                    className="w-full bg-transparent border-b border-[var(--border)] py-2 text-sm font-mono outline-none focus:border-white transition-colors text-white"
                  />
                </Field>
                <Field
                  label="Exposed Port"
                ...
                <Field
                  ...
                  sub="Used by ALB to verify health"
                >
                  <input
                    value={healthCheck}
                ...
                <div className="grid grid-cols-2 gap-6">
                  <Field label="CPU" sub="vCPU units">
                    <select
                      value={cpu}
                      onChange={(e) => setCpu(e.target.value)}
                      className="w-full bg-transparent border-b border-[var(--border)] py-2 text-sm font-mono outline-none focus:border-white cursor-pointer appearance-none text-white"
                    >
                      {CPU_OPTIONS.map((o) => (
                        <option
                          key={o.value}
                ...
                <Field label="Environment Variables" sub="Injected at runtime">
                  <div className="space-y-px mt-2">
                    {envVars.map((e, i) => (
                      ...
                      >
                        <input
                          ...
                          className="h-9 bg-[var(--bg)] px-3 font-mono text-xs text-white outline-none"
                        />
                        <input
                          type="text"
                          placeholder="value"
                        ...
                        <button
                          ...
                          className="h-9 w-9 bg-[var(--bg)] hover:bg-red-950 text-[#444] hover:text-red-400 font-mono text-xs"
                        >
                          ×
                        </button>
                      </div>
                    ...
                    <button
                      onClick={addEnvVar}
                      className="w-full h-9 border border-dashed border-[var(--border)] font-mono text-[9px] text-[#444] hover:text-white uppercase tracking-widest"
                    >
                      + Add Variable
                    </button>
                  </div>
              ...
              <div className="pt-6 border-t border-[var(--border)] space-y-3">
                <button
                  ...
                  className="w-full bg-white text-black py-4 font-bold font-mono text-[11px] uppercase tracking-[0.2em] hover:bg-[#e5e5e5] transition-all disabled:opacity-40"
                >
                  {checkingDocker
        ...
        <div className="w-full max-w-2xl border border-[var(--border)] bg-[var(--bg)] flex flex-col relative z-10 shadow-2xl">
          <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between bg-[#050505]">
            <div className="flex items-center gap-2">
              <div
                className={`w-1.5 h-1.5 rounded-full ${selectedRepo ? "bg-white" : "bg-[#1a1a1a]"}`}
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-[var(--border)]">
              ...
              <div className="p-8 bg-[#080808] flex flex-col gap-8">
                <div className="font-mono text-[10px] leading-relaxed space-y-1 bg-black border border-[var(--border)] p-5">
                  <p className="text-blue-400">manifest = {"{"}</p>
                ...
                <div
                  className={`flex items-center gap-3 px-4 py-3 border ${checkingDocker ? "border-[#111] opacity-50" : hasDockerfile ? "border-green-900/40 bg-green-900/5" : "border-red-900/40 bg-red-900/5"}`}
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${checkingDocker ? "bg-[#333] animate-pulse" : hasDockerfile ? "bg-[#10b981]" : "bg-red-500"}`}
                  />
                  <p
                    className={`font-mono text-[8px] uppercase tracking-wider ${checkingDocker ? "text-[#333]" : hasDockerfile ? "text-[#10b981]/70" : "text-red-500/70"}`}
                  >
                    {checkingDocker
                      ? "Checking_Dockerfile..."
                      : hasDockerfile
...
function StepDot({
  ...
  return (
    <div
      className={`w-4 h-4 flex items-center justify-center font-mono text-[8px] font-bold border ${done ? "bg-[#10b981] border-[#10b981] text-black" : active ? "bg-white border-white text-black" : "border-[#333] text-[#333]"}`}
    >
      {done ? "✓" : n}
    </div>
  );
